...
def seed_ensemble(X_train, y_train, X_test, y_true_eval, inverse, value_name: str, meta_df: pd.DataFrame) -> pd.DataFrame:
    preds = []
...
def run_split(split_name: str, df: pd.DataFrame, X: pd.DataFrame, train_idx, test_idx) -> dict:
    ...
    if split_name == "scaffold":
        ...
        seed_ensemble(X_train, y_wave_train, X_test, y_wave_test, None, "wavelength", split_df_test).to_csv(
            config.METRICS_DIR / "wavelength_uncertainty.csv", index=False
        )
        seed_ensemble(X_train, y_plqy_train, X_test, y_plqy_test, None, "plqy", split_df_test).to_csv(
...
def main() -> None:
    ...
    dataset_stats["unique_scaffolds"] = len({get_scaffold(s) for s in df["canonical_smiles"]})
